 ...
 * 2. 类型
   2.1 TCP半连接
   2.2 HTTP连接：是正常的请求，但每个服务器所提供的服务都是有限的
   2.3 DNS：DNS每一次访问的负载很小，其带宽容量不会那么大
 ...
 * samesite
 * // ===
   samesite - 禁止第三方网站带cookie - 即：只有来自同一站点，才能携带samesite的cookie
   samesite: strict; - 任何请求都不能携带cookie，比如一个链接到自己的网站，这也是不允许携带cookie
   samesite: lax; - 某些请求都能携带cookie，比如链接。【ajax和form是不允许携带cookie】

   缺点：兼容性差，目前只有chrome和opera支持
 * === //
 */
 ...
 * // ===
 * 1. 攻击原理
   1.1 用户登录A网站，浏览器存储cookie
   1.2 在同一个浏览器打开攻击者网站，即使无法获取cookie，但从浏览器向A网站发出的任何请求中，都会携带其cookie
 * 2. 危害
   2.1 利用用户登录态：盗取用户资金 （转账/消费）
   2.2 用户不知情：比如冒充用户发帖子
   2.3 完成业务请求：损坏网站名誉
 * 3. 攻击特征 + 防御手段
   3.1 B网站向A网站请求, 带A网站的cookie ==》 samesite
       设置samesite后，即使登录态仍存在，但是攻击者网站是无法获取到A网站的cookie
   3.2 referer为B网站 ==》 验证referer
   3.3 绕过A网站前端，直接发送请求给后端 ==》 在前端页面加入验证信息 - 验证码（ccap模块实现） / csrf_token(随机值)
 * === //
 */
